...
the rôle or meaning of private routines, although of course, they should be
documented within the Implementation Guide. Private routines and variables are
...
such routines. Use the Implementation Guide as a reference for what such
routines/variables do, and if you make any modifications, you must update the
Implementation Guide and note the changes within the source code as well. The
subprogram template is there to remind you to document all side-effects of each
routine!
...
These should be avoided wherever possible as they hinder portability. They should
only be used in the `Utility` modules for machine specific code. The one exception
to this rule, is the use of a single `#ifdef debug` block at the start
of each routine to check for conditions of usage. For this reason, all module
filenames should be given the extension `.F90` not `.f90` to enable
preproccesing on the widest range of compilers available.
...
routines are not specified! The function of the auxiliary module must, of
course, be documented in the Implementation Guide.
...
### Use `ModuleTemplate.F90`

The `ModuleTemplate.F90` file gives the recommended layout for a module and for
a subprogram. In particular, note the layout and contents of the comment header,
and the order of declarations. Everything is to be private unless explicitly
specified as public! This also makes it simple to check the code against the
specification.
...
the Bitbucket repository has been passed through Emacs f90-mode indentation at the
finished stage. To assist in Emacs usage, the `ModuleTemplate.F90` file contains
Emacs directives to set f90-mode, etc.
...
both internally and in this Implementation Guide, with a discussion as to the
design decisions made.


### Be aware of vectorisation

When coding, be aware that the code is often executed upon vector architectures.
Therefore code with a style that enables good vectorisation - e.g. don't put
`if` statements inside nested loops wherever possible.
...
Each module, based upon `ModuleTemplate.F90`, is to have a header block
describing its overall purpose and function. Note that this header block also
contains RCS directives to log changes to the code.
...
documentation has been supplied for inclusion in the Implementation Guide. Each
module is to be described by one chapter of the Implementation Guide, explaining
the function of the module and each routine, the physics behind the code, the
algorithms used, the key variables and data structures, and appropriate
references to the literature.

<!--  A documentation template is included in appendix  B. -->
...
execute correctly in the CI system. The purpose of these tests is to verify the correct
operation of every specified routine in the module.
...
! -*- mode: F90 ; mode: font-lock ; column-number-mode: true ; vc-back-end: mercurial -*-
!=============================================================================!
!                          D U M M Y                                          !
!=============================================================================!
!                                                                             !
