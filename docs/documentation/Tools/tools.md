...
* [MDANSE](https://github.com/ISISNeutronMuon/MDANSE/) (''Molecular Dynamics Analysis for Neutron Scattering Experiments'') is a python application for molecular dynamics trajectory analysis. Apart from general analysis of structure and dynamics (van Hove functions, vibrational density of states), it is mainly used for computing properties that can be directly compared with neutron scattering experiments such as the coherent and incoherent intermediate scattering functions and their Fourier transforms, the elastic incoherent structure factor, the static coherent structure factor or the radial distribution function.
